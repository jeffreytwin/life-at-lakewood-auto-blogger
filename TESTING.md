...
| 3.5 | Check KPI cards | "Total Posts" and "Published" counts are correct |
...
| 5.9 | Select 1+ keywords → click "Next" | Advances to Step 1 |
| 5.10 | Click "Next" with no keywords selected | Button is disabled or shows warning |
...
| # | Test | Expected Result |
|---|------|----------------|
| 7.1 | Generation starts | Progress animation plays through stages (Fetching competitors → Outlining → Writing → etc.) |
| 7.2 | Open browser Network tab | POST requests to `/api/claude/generate` for each article |
| 7.3 | Generation completes | Progress hits 100%, auto-advances to Step 3 |
| 7.4 | Check response data | Each article has `seoTitle`, `metaDescription`, `slug`, and `sections` array |
| 7.5 | **If Claude API fails:** | Mock article content used as fallback, no crash |
...
| 8.6 | Click "Approve All" | Advances to Step 4 |
...
| 9.5 | **Verify in Wix:** go to Wix dashboard → Blog → Drafts | New draft posts appear with correct titles and content |
